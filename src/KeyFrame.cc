...
KeyFrame::KeyFrame():
        ...
        mnMaxY(0), mnImageWidth(0), mnImageHeight(0), /*mK(NULL),*/  mPrevKF(static_cast<KeyFrame*>(NULL)), mNextKF(static_cast<KeyFrame*>(NULL)), mbFirstConnection(true), mpParent(NULL), mbNotErase(false),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mnMaxY(F.mnMaxY), mnImageWidth(F.imgLeft.cols), mnImageHeight(F.imgLeft.rows), mK(F.mK), mPrevKF(NULL), mNextKF(NULL), mpImuPreintegrated(F.mpImuPreintegrated),
...
cv::Size KeyFrame::ImageSize() const
{
    return cv::Size(mnImageWidth, mnImageHeight);
}
